...
LRESULT Workspace::OnCreate( LPCREATESTRUCT crst )
{
	CreateCells();
...
void Workspace::OnDraw( HDC hdc )
{
	DrawRulers( hdc );
	DrawCells( hdc );
}

LRESULT Workspace::OnLButtonDown( DWORD dwKeyIndicator, WORD x, WORD y )
{
	...
	if ( itCell != m_cells.end() )
	{
		if ( OnCellLClicked( itCell.base() ) )
			::InvalidateRect( GetHwnd(), itCell->GetRectPtr(), FALSE );
	}

	return 0;
}

LRESULT Workspace::OnRButtonDown( DWORD dwKeyIndicator, WORD x, WORD y )
{
	std::vector<Cell>::iterator itCell = 0;
