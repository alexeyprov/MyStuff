...
'Option Explicit
' The class declared in this shared library are based on the following Parameters format.
'
'   0 * Information Message
'   1 * Error Message(s)
'   2 * Default Fields * 2 dimensional array
'   3 * Foreign Key Field Name
'   4  * Reserved for future use
'   5  * Reserved for future use

' Name: CreateTransitPointParamsObj
' Purpose: This is the factory function for the TransitPointParams class, as the Global
' object does not share VBScript classes in the current version.
' -------------------------------------------------------------------------------------------
' Revision# Date            Author  Description
' --------  ----            ------  -----------
'           Dec 20, 99      JH      Initial version
' 5.0       10/31/2002      DY      Make the 6-th reserved parameter as a extended reserved
'                                   parameter list.
' -------------------------------------------------------------------------------------------
' Revisions
'
Function CreateTransitPointParamsObj()
    Set CreateTransitPointParamsObj = New TransitPointParams
End Function
...
Function CreateCustomDialogBox()
    Set CreateCustomDialogBox = New CustomDialogBox
End Function
...
' ------------------------------------------------------------------------------------------
Function CMSDialogRadioEx(strHTMLText, strTitle, strBtnTexts, iDefaultBtn)
    Dim lngRet
    Dim strReturn
    Dim rfrmRadioButton
    ...
    Set rfrmRadioButtonMain = UIMaster.CreateFormParam()
    Set rfrmRadioButtons = UIMaster.CreateFormParams()
    Set rfrmParamMain = UIMaster.CreateFormParams()
    With rfrmParamMain
        If IsArray(strBtnTexts) = True Then
            For lngRet = 0 To UBound(strBtnTexts)
                Set rfrmRadioButton = UIMaster.CreateFormParam()
...
' ------------------------------------------------------------------------------------------
Function CMSDialog(strHTMLText, strTitle, strBtnTexts)
    Dim rdlgParam
    Dim lngRet
    Dim strReturn
    
    Set rdlgParam = UIMaster.CreateDialogParam
    With rdlgParam
        If IsArray(strBtnTexts) = True Then
            For lngRet = 0 To UBound(strBtnTexts)
...
' 1.0           01/12/2000  DY      Initial version
' ------------------------------------------------------------------------------------------
Function CMSMsgBox(strPrompt, intButtons, strTitle)
    Const strdOK = "OK"
    ...
    Set rdltLangDict = UIMaster.RSysClient.GetLDGroup("Common")
...
' 1.0           01/12/2000  DY       Initial version
' ------------------------------------------------------------------------------------------
Function NewActivity(blnShowModal, vntContactId, vntLeadId, vntCompanyId, vntOpportunityId, vntMarketingProject)
    ...
    With objParam
        If Not IsNull(vntCompanyId) Then .AddDefaultField strfCOMPANY, vntCompanyId
        If Not IsNull(vntContactId) Then .AddDefaultField strfCONTACT, vntContactId
        If Not IsNull(vntLeadId) Then .AddDefaultField strfLEAD_ID, vntLeadId
        If Not IsNull(vntOpportunityId) Then .AddDefaultField strfOPPORTUNITY, vntOpportunityId
        If Not IsNull(vntMarketingProject) Then .AddDefaultField strfMARKETING_PROJECT, vntMarketingProject
...
' ------------------------------------------------------------------------------------------
Function EqualValues(vntValue1, vntValue2)
    On Error Resume Next
...
' ------------------------------------------------------------------------------------------
Function ApplyActionPlan(strObject, vntPartnerId, vntPartnerContactId, vntManagerId, vntParameters)
    ...
    With UIMaster.RSysClient
        Set rfrmForm = .GetForm(strrACTION_PLAN)
        Set rdltLangDict = .GetLDGroup(strgACTION_PLAN)
    End With
    ...
    If Err.Number <> 0 Then
        ...
        Exit Function
    End If
    Set rstRecordset = objParam.GetUserDefParam(1, vntArguments)
...
' 1.0           02/15/2000  DY      Initial version
' ------------------------------------------------------------------------------------------
Function CMSDialogRadio(strHTMLText, strTitle, strBtnTexts)
    Dim lngRet
    Dim strReturn
    Dim rfrmRadioButton
    Dim rfrmRadioButtons
    ...
    With rfrmParamMain
        Set rfrmRadioTitle = UIMaster.CreateFormParam()
...
'**********************************************************
Sub ExpandRequiredSegments(rfrmForm, vntExceptions)
...
'1.0        March 13, 2000 TS     Initial version
'**********************************************************
Sub ExpandTopSegments(rfrmForm, intNumSegments, vntRecordsets)
    ...
    For intSegmentCounter = 0 To intMaxSegments - 1
        With rfrmForm.Segments(intSegmentCounter)
            If .IsTitleHidden = False Then
                If .IsSecondary = True Then
                    Set rstSecondary = rfrmForm.SecondaryFromVariantArray(vntRecordsets, .SegmentName)
...
'**********************************************************
Function PrePendEmailBody(strBody)
...
' iType -  0 - Quote
'          1 - Order
Sub InitDirtyFlag(iWebTabs, rstItems, rstSolutions, strType)
...
Sub UpdateDirtyFlag(rstItems, rstSolutions, iType)
...
Function GetItemFlag(vntId, arrItems)
...
Function OkToChangeTab(strTabName, strType)
    ...
    If arrDirtyWebTabs(iTab) = True Then
        With UIMaster.RSysClient.GetLDGroup("Quote")
            ...
            Else
                Exit Function
            End If
            strBtnTexts = Array(.GetText("Yes"), .GetText("No"))
        End With
...
' ------------------------------------------------------------------------------------------
Function CalculateTimeDiff(Interval, StartDate, StartTime, _
            EndDate, EndTime)
...
' ------------------------------------------------------------------------------------------
Sub PushUIStack(objUIMaster)
Dim intUB
On Error Resume Next
    If IsEmpty(arrUIStack) = False Then
        intUB = UBound(arrUIStack)
        ReDim Preserve arrUIStack(intUB + 1)
        Set arrUIStack(intUB + 1) = objUIMaster
    Else
        arrUIStack = Array(objUIMaster)
    End If
...
On Error Resume Next
    ...
    End If
    If intUB >= 0 Then
        Set GetCurUI = arrUIStack(intUB)
    Else
         Set GetCurUI = Nothing
    End If
End Function
...
' 4.0           05/05/2001  CT      Initial version
' ------------------------------------------------------------------------------------------
Sub ShowFKForm(UIMaster, vntFormId, vntParameters)

On Error Resume Next
    PushUIStack UIMaster
    UIMaster.ShowFormModal vntFormId, Null, vntParameters
    PopUIStack
    If Err.Number <> 0 Then
        Exit Sub
    End If
...
' ------------------------------------------------------------------------------------------
Function CheckCompanyLeadContactFK(UIMaster, vntExistParameters)
    ...
    Set eventObj = UIMaster.RUICenter.FormEventObj
    Set sysClnt = UIMaster.RSysClient

     Set tb = UIMaster.RUICenter.Form.Tabs.Item(eventObj.TabIndex)
     Set seg = tb.Segments.Item(eventObj.SegmentIndex)
     Set fld = seg.FormFields.Item(eventObj.FieldIndex)
     tableId = fld.ForeignField.Table.tableId
    
    'MsgBox (sysClnt.IdToString(tableId))
    
    Set security = sysClnt.GetUserSecurity
    ...
    If (strTableName = "Lead_" Or _
        ...
        strTableName = "Company") And IsNull(recordId) Then
        If strTableName = "Lead_" Then
            Set rForm = sysClnt.GetForm("Lead")
            formId = rForm.formId
        End If
        'set strFieldName to vntParameter(3)
        If IsNull(vntExistParameters) = True Then
            Set objParam = CreateTransitPointParamsObj()
...
' ------------------------------------------------------------------------------------------
Function SaveNewRecord(vntTableId, rstDuplicates, strFormName, strFKFieldName)
...
Dim objCurUI
    ...
    On Error Resume Next
        ...
        Case strfrmCOMPANY
            Set rldtLangDict = UIMaster.RSysClient.GetLDGroup(strgCOMPANY)
       ...
       Case strfrmCONTACT
            Set rldtLangDict = UIMaster.RSysClient.GetLDGroup(strgCONTACT)
        ...
        Case strfrmLEAD
            Set rldtLangDict = UIMaster.RSysClient.GetLDGroup(strgLEAD)
        ...
        While blnSelectAnotherOne = True
            ...
            If UBound(arrRecord_Id) < 0 Then
                ...
                Else
                    SaveNewRecord = False
                End If
            ElseIf Not IsNull(arrRecord_Id(0)) Then
                ...
                If arrRecord_Id(0) <> 2 Then
                    If CMSMsgBox(strMsgIsThis, vbYesNo, strTitle) = vbYes Then
                        If IsEmpty(arrUIStack) = False And IsEmpty(strFKFieldName) = False Then
                                Set objCurUI = GetCurUI
...
' 4.0           05/05/2001  CT      Initial version
' ------------------------------------------------------------------------------------------
Sub SetNewFKId(strFKFieldName, vntNewRecId)
On Error Resume Next
Dim objCurUI
        If IsEmpty(arrUIStack) = False And IsEmpty(strFKFieldName) = False Then
                Set objCurUI = GetCurUI
...
' ------------------------------------------------------------------------------------------
Sub SetRequiredFields(vntRequiredFields)
    ...
    If IsEmpty(vntRequiredFields) = True Then Exit Sub
...
Sub OnPortalLoaded(ParameterList)
    ...
    If IsNull(UIMaster.RSysClient.EmployeeId) Then Exit Sub

    Set rfrmForm = UIMaster.RSysClient.GetForm("Employee")
    ...
    vntRecordsets = rfrmForm.doLoadFormData(UIMaster.RSysClient.EmployeeId, Null)
    
    Dim objButton
    Dim objLangGroup
    Dim strText
    Set objLangGroup = UIMaster.RSysClient.GetLDGroup("Duty Manager")
    ...
    End If
    Set objButton = UIMaster.RUITop.AddButton(0, strText, 9999)
...
Sub OnModelessWindowClose()
  ...
  If IsObject(UIMaster.Master.Global.objUIMasterFullTextSearchModelessWindow) Then
    If UIMaster.Master.Global.objUIMasterFullTextSearchModelessWindow Is UIMaster Then
      Set UIMaster.Master.Global.objUIMasterFullTextSearchModelessWindow = Nothing
      UIMaster.Master.Global.blnDisplayRecordFromListInModelessWindow = False
    End If
  End If

  If IsObject(UIMaster.Master.Global.objUIMasterModelessWindowFromSearch) Then
    If UIMaster.Master.Global.objUIMasterModelessWindowFromSearch Is UIMaster Then
      Set UIMaster.Master.Global.objUIMasterModelessWindowFromSearch = Nothing
      UIMaster.Master.Global.objUIMasterModelessWindowFromSearch = Empty
    End If
  End If
...
Sub IsLastWindow()
    ...
    If UBound(UIMaster.AllMasterChildren) < 0 And UIMaster.Master.window Is Nothing Then
        ...
        vntRecordsets(0).Fields("Duty_Manager").Value = False
            Call rfrmForm.doSaveFormData(vntRecordsets, Null)
        End If
    End If
End Sub
...
Function SearchResultsEmailLinkClickHandler()
    ...
    Else
        ...
        Set objSearch = UIMaster.RUICenter
        ...
        UIMaster.ShowFormModal "Outbound Email", Null, vntParameters
    End If
...
Function CreateModelessWindowDef()
  ...
  Set ruiModelessWindowDef = UIMaster.CreateModelessWindowDef
  ...
  ruiModelessWindowDef.Height = CLng(0.85 * window.Screen.availHeight)
  ruiModelessWindowDef.Width = CLng(0.95 * window.Screen.availWidth)
  Set CreateModelessWindowDef = ruiModelessWindowDef
...
Function OpenModelessFullTextSearchWindow()
    ...
    With UIMaster.RUICenter.SearchEventObj
        ...
        If blnUseOpenRecordLogic = True Then 'A record Id (either PK or FK) was clicked
            ...
            If IsEmpty(UIMaster.Master.Global.objUIMasterModelessWindowFromSearch) Or UBound(UIMaster.AllMasterChildren) = -1 Then
                Set ruiModelessWindowDef = CreateModelessWindowDef
                Set UIMaster.Master.Global.objUIMasterModelessWindowFromSearch = UIMaster.ShowFormByTableModeless(vntTableId, vntRecordId, ruiModelessWindowDef, Null)
            Else
                On Error Resume Next
                UIMaster.Master.Global.objUIMasterModelessWindowFromSearch.ShowFormByTable 2, vntTableId, vntRecordId, Null
...
Function OpenModalFullTextSearchWindow()
    ...
    With UIMaster.RUICenter.SearchEventObj
        ...
        If blnUseOpenRecordLogic = True Then 'A record Id (either PK or FK) was clicked
            vntRecordId = .Value
            vntTableId = .TargetTable.tableId
            Set rduSecurity = UIMaster.RSysClient.GetUserSecurity
            vntFormId = rduSecurity.DefaultFormId(vntTableId)
            ' This method does not works, so that we have to use ShowFormModal
            'UIMaster.ShowFormByTable actionAskUser, vntTableId, vntRecordId, Null
            UIMaster.ShowFormModal vntFormId, vntRecordId, Null
        Else 'An email, fax, URL or unknown type link was selected
            If .Field.DataDictionary.DataAttribute = 3 Then 'metaTextEmail
...
'                                    modal form.
'-------------------------------------------------------------------------------------------
Sub CSSFullTextSearch(strSearchText)
    ...
    With UIMaster
        ...
        If .RUICenter.IsModal Then
            ...
            '.ShowCenterReference actionAskUser, objSearchFactory, Null
             Set objParam = .Master.Global.CreateTransitPointParamsObj()
             objParam.SetUserDefParam 1, strSearchText
             vntParameters = objParam.ConstructParams
             .ShowFormModal "Temp Incident", Null, vntParameters
        Else
            Set objSearchFactory = UIMaster.CreateCenterReference("search")
            With objSearchFactory
                .SearchType = 5 'searchTypeInContext
                If Not IsNull(strSearchText) Then .SearchText = strSearchText
                ...
                If Not (IsNull(strSearchText)) Then
            ...
            With ruiModelessWindowDef
                ...
                .Height = CLng(0.85 * window.Screen.availHeight)
                .Width = CLng(0.95 * window.Screen.availWidth)
            End With
            .ShowCenterReferenceModeless actionAskUser, objSearchFactory, ruiModelessWindowDef, Null
        End If
    End With
...
' ------------------------------------------------------------------------------------------
Sub CMSErrorInitialize(objErr)
    If objErr.Number = 0 Then
        Set gobjCMSError = Nothing
    Else
        Set gobjCMSError = New CMSErrorObject
...
Sub CMSErrorHandling()
    ...
    ElseIf lngError = E_RDA_REQUIRED_REFRESH Then
        ...
        With UIMaster.RSysClient.GetLDGroup("Common")
            ...
            If IsEmpty(strWarning) Then strWarning = "Warning"
            If IsEmpty(strRefresh) Then strWarning = "Do you want to refresh?"
        End With
...
' ------------------------------------------------------------------------------------------
Sub ShowSystemErrorMessage(objErr)
    Const lngERR_APPDEV_START_NUMBER = 10000
    Const lngERR_APPDEV_END_NUMBER = 29999
    Const lngERR_CMS_EXPAN_START_NUMBER = 40000
    Const lngERR_CMS_EXPAN_END_NUMBER = 49151
...
' ------------------------------------------------------------------------------------------
Public Function GetForeignFieldText(strTab, strSegment, strField)
    Dim rduRecord
        
    On Error Resume Next
    
    If IsNull(strTab) Then strTab = ""
    If Len(strTab) = 0 Then
        Set rduRecord = UIMaster.RUICenter.GetForeignField(strSegment, strField)
    Else
        Set rduRecord = UIMaster.RUICenter.GetForeignFieldEx(strTab, strSegment, strField)
    End If
...
'               07/29/2002  DY      Initial version
' ------------------------------------------------------------------------------------------
Public Function GetForeignFieldTextEx(strField)
    Dim rduRecord
    Dim rdaTab
    Dim rdaSegment
    Dim rdaFormField
    ...
    Set rdaForm = UIMaster.RUICenter.Form
...
'               07/29/2002  DY      Initial version
' ------------------------------------------------------------------------------------------
Public Function GetDisconnectedFieldNameEx(strFieldOrTitle)
    Dim rduRecord
    Dim rdaTab
    Dim rdaSegment
    Dim rdaFormField
...
' ------------------------------------------------------------------------------------------
Function GetComboChoiceText(strBaseString, strFieldName, strTableName)
    Dim strTable
    On Error Resume Next
    GetComboChoiceText = ""
    If IsNull(strTableName) Then strTableName = ""
...
'**********************************************************
Sub OnEditActivity(vntActivityId)

  Const strrTEMP_ACTIVITY = "Temp Activity"

  UIMaster.ShowFormModal strrTEMP_ACTIVITY, vntActivityId, Null
End Sub
...
'**********************************************************
Function OnNewAppointment(intActivityType, dtmStartDateTime, dtmEndDateTime, vntAssignedToEmployeeId, vntAssignedByEmployeeId)
  ...
  Set objParams = CreateTransitPointParamsObj()
  If Not (IsNull(dtmStartDateTime)) Then
    ...
    End If
  End If
  If Not (IsNull(dtmEndDateTime)) Then
...
' or null, if the user canceled - to the calling Calendar function.
'Assumptions:
'Effects:
'Inputs: intActivityType - an integer representing the activity type.
...
'1.0        September 27, 2001 TS         Initial version
'**********************************************************
Function OnNewTask(intActivityType, dtmStartDate, vntAssignedToEmployeeId, vntAssignedByEmployeeId)
...
'1.0        September 27, 2001 TS         Initial version
'**********************************************************
Function OnNewActivity(dtmStartDate, vntAssignedToEmployeeId, vntAssignedByEmployeeId)
...
'1.0        September 5, 2001 TS         Initial version
'**********************************************************
Function NewActivity2(blnShowModal, vntParameters)
    ...
    With UIMaster.RSysClient.GetLDGroup("Activity Management")
        ...
        strBtnTexts = Array(.GetText("Call Button"), .GetText("Literature Fulfillment Button"), _
            .GetText("Meeting Button"), .GetText("Message Button"), .GetText("Note Button"), _
            .GetText("To-Do Button"))
        intAppTypes = Array(intAPP_TYPE_CALL, intAPP_TYPE_LITERATURE, intAPP_TYPE_MEETING, _
...
'1.0        September 5, 2001 TS         Initial version
'**********************************************************
Function MapActivityTypeToFormName(intActivityType)
...
'1.0        September 27, 2001 TS         Initial version
'**********************************************************
Function URLEncodeParameter(strURLParameter)
...
'                                   to maintain.
' -------------------------------------------------------------------------------------------------
Sub StartEmail(strTo, strSubject, strBody, strTableName, vntRecordId)
    ...
    Else
        ...
        vntParameters = objParams.ConstructParams()
        UIMaster.ShowFormModal strOUTBOUND_EMAIL, Null, vntParameters
    End If
End Sub
...
Function OnEmailLabelClick()
   ...
   Else
       ...
       Dim strTableName
       Dim vntId
       Set objFormContext = UIMaster.RUICenter.FormEventObj
       Set objParams = CreateTransitPointParamsObj()
