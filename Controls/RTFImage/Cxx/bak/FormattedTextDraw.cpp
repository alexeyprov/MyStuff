...
{
	m_rcClient = *prc;
	m_sizelExtent.cx = MAP_PIX_TO_LOGHIM((m_rcClient.right - m_rcClient.left) - 2, nPixelsPerInchX);
	m_sizelExtent.cy = MAP_PIX_TO_LOGHIM((m_rcClient.bottom - m_rcClient.top) - 2, nPixelsPerInchY);
...
{
	return ::ReleaseDC(m_hWndParent, hdc);
}
...
{
	::InvalidateRect(m_hWndParent, prc, fMode);
}
...
void CFormattedTextDraw::TxViewChange(BOOL fUpdate)
{
	if(m_spTextServices)
	{
		...
		m_spTextServices->TxDraw(
				...
				NULL, 				   	// Target device HDC
				(RECTL *) &m_rcClient,	// Bounding client rectangle
				NULL, 					// Clipping rectangle for metafiles
				(RECT *) NULL,			// Update rectangle
				NULL, 	   				// Call back function
				NULL,					// Call back parameter
