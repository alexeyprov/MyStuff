...
DWORD CALLBACK EditStreamInCallback(DWORD dwCookie, LPBYTE pbBuff, LONG cb, LONG *pcb)
{
	...
	char* bstrStart = ((char*)pCookie->bstrText) + pCookie->dwCount;
	CopyMemory(pbBuff, bstrStart, *pcb);
	pCookie->dwCount += *pcb;
...
HRESULT CFormattedTextDraw::Draw(void *hdcDraw, RECT *prc)
{
	...
	//FillRect((HDC)hdcDraw, prc, (HBRUSH) (COLOR_APPWORKSPACE));
	return S_OK;
}
...
HRESULT CFormattedTextDraw::CreateTextServicesObject()
{
	...
	if (hr == S_OK) {
		...
		CRichEditCallback* richedtCallback = new CRichEditCallback;
		LRESULT lresult;
		hr = m_spTextServices->TxSendMessage(EM_SETOLECALLBACK, 0, 
			reinterpret_cast<LPARAM>(static_cast<IRichEditOleCallback*>(richedtCallback)), &lresult);
